...
async function deleteOrder(request, env, orderId) {
  ...
  const order = await env.DB.prepare('SELECT id FROM orders WHERE id = ? AND user_id = ?').bind(id, user.id).first();
  if (!order) return error('주문을 찾을 수 없습니다.', 404);
  await env.DB.batch([
    env.DB.prepare('DELETE FROM order_items WHERE order_id = ?').bind(id),
    env.DB.prepare('DELETE FROM orders WHERE id = ? AND user_id = ?').bind(id, user.id),
  ]);
  return json({ ok: true });
}
...
async function productIntroduction(request, env) {
...
export default {
  async fetch(request, env) {
    ...
    const csp = pathname.startsWith('/orders') ? "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' https://js.tosspayments.com; connect-src 'self' https://api.tosspayments.com https://apigw.tosspayments.com https://apigw-sandbox.tosspayments.com https://connect.tosspayments.com https://event.tosspayments.com https://log.tosspayments.com https://payment-widget.tosspayments.com https://payment-gateway.tosspayments.com https://payment-gateway-sandbox.tosspayments.com; frame-src https://payment-widget.tosspayments.com https://payment-gateway.tosspayments.com https://payment-gateway-sandbox.tosspayments.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'" : "default-src 'self'; img-src 'self' data:; style-src 'self'; script-src 'self' https://js.tosspayments.com; connect-src 'self' https://api.tosspayments.com https://apigw.tosspayments.com https://connect.tosspayments.com https://event.tosspayments.com https://log.tosspayments.com https://payment-widget.tosspayments.com https://payment-gateway.tosspayments.com https://payment-gateway-sandbox.tosspayments.com; frame-src https://payment-widget.tosspayments.com https://payment-gateway.tosspayments.com https://payment-gateway-sandbox.tosspayments.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'"; headers.set('Content-Security-Policy', csp);
